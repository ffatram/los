...
class cetak extends Controller
{
    ...
    // master fungsi untuk cetak word

    public function cetaktempalteword($filetemplate, $keyword, $jaminan = [], $images = '', $namafilelast)
    {


        // Menggunakan file gambar.docx sebagai template
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('cetak/cetakbaru/' . $filetemplate);
        try {
            // Melakukan penggantian nilai berdasarkan array
            foreach ($keyword as $key => $value) {
                $templateProcessor->setValue($key, $value);
            }


            $maxJaminan = count($jaminan);

            $templateProcessor->cloneRow('jaminan', $maxJaminan);
            foreach ($jaminan as $indeks => $nilai) {
                $templateProcessor->setValue('jaminan#' . $indeks, $nilai);
            }



            // Path gambar yang akan dimasukkan
            $imagePath = 'upload/' . isset($images) ? $images : '';

            // Mengganti placeholder dengan gambar
            $templateProcessor->setImageValue('gambar', array('path' => $imagePath, 'width' => 300, 'height' => 200));

            // Simpan hasilnya ke dalam file output.docx
            $outputFilePath = 'cetak/cetakbaru/' . '_download_' . $filetemplate;
            $templateProcessor->saveAs($outputFilePath);

            // Setel header untuk pengunduhan
            header('Content-Type: application/octet-stream');
            header('Content-Disposition: attachment; filename="' . $namafilelast . '"');
    ...
    public function cetak_pk($id)
    {
        ...
        $data_cetak_pk =  $this->format_data_cetak_pk($id);


        $nama_pemohon = $data_cetak_pk['datatambahan']['nama_pemohon'];

        $ketsuamiistri = $data_cetak_pk['datatambahan']['jenis_kelamin_debitur'];

        ($ketsuamiistri == "PRIA") ? "ISTRI" : "SUAMI";

        $filetemplate = '';
        // $filetemplate = 'PK ' . $data_cetak_pk['tambahan']['jenis_no_pk'] . ' ';
        if ($data_cetak_pk['datatambahan']['tgl_surat_kuasa_pasangan'] != "") {

            $filetemplate = "PK " . $data_cetak_pk['datatambahan']['tipe_pejabat'] . " Surat Kuasa.docx";
            $datavalue['pasangan'] = $ketsuamiistri;
            $datavalue['tglsuratkuasa'] = $data_cetak_pk['datatambahan']['tgl_surat_kuasa_pasangan'];
        } else {
            if ($data_cetak_pk['datatambahan']['nama_pasangan'] != '' || $data_cetak_pk['datatambahan']['nama_pasangan'] != null) {
                $filetemplate = "PK " . $data_cetak_pk['datatambahan']['tipe_pejabat'] . " Kawin.docx";
            } else {
                $filetemplate = "PK " . $data_cetak_pk['datatambahan']['tipe_pejabat'] . " Lajang.docx";
            }
        }


        $jaminan = $data_cetak_pk['datatambahan']['jaminan'];
        $keyword = $data_cetak_pk['keyword'];
        $images = '';
        $namafile = 'PK' . " " . $nama_pemohon . ".docx";

        $this->cetaktempalteword($filetemplate, $keyword, $jaminan, $images, $namafile);
    }
    ...
    public function cetak_permohonankreditonline($noreg)
    {
        ...
        try {
            ...
            $data->setValue('aset_yang_dimiliki', number_format(htmlspecialchars($i['aset_yang_dimiliki']), 0, ',', '.'));
    ...
    public function getmodel($id)
    {
        ...
        $data['cetak_pk'] = $this->model('m_cetak')->cetak_pk($id);
        $data['ref_pejabat'] = $this->model('m_cetak')->ref_pejabat($data['cetak_pk']['pejabat_ttd']);
        $data['ref_sk_limit'] = $this->model('m_cetak')->ref_sk_limit($data['cetak_pk']['id_sk']);
        $data['tbl_cabang'] = $this->model('m_cetak')->tbl_cabang($_COOKIE['kode_cabang']);
        $data['tbl_permohon_kredit'] = $this->model('m_cetak')->get_tbl_permohon_kredit($id);
        $data['nama_pemohon'] = $data['tbl_permohon_kredit']['nama_pemohon'];
        $data['format_tanggal_indo'] = new format_tanggal_indo();
        $data['terbilang'] =  new terbilang();
        $data['jenis_sppk'] = $data['cetak_pk']['jenis_no_pk'];
        $data['tbl_ref_jenis_no_pk_where_jenis'] = $this->model('m_ref_jenis_no_pk')->get_jenis_berkas_pk($data['cetak_pk']['jenis_no_pk']);
    ...
    public function getdatavalue($id)
    {
        ...
        $keyword = [
            'nomor_pk' => $data['cetak_pk']['no_pk'],
            'sebutan' =>  $data['ref_pejabat']['sebutan'],
            'nama_pejabat' =>  $data['ref_pejabat']['nama_pejabat'],
            'tempat_lahir_pejabat' =>  $data['ref_pejabat']['tempat_lahir'],
            'kota_pejabat' =>  $data['ref_pejabat']['kota_pejabat'],
            'alamat_pejabat' =>  $data['ref_pejabat']['alamat'],
            'tanggal_lahir_pejabat' => $data['format_tanggal_indo']->format_tgl($data['ref_pejabat']['tanggal_lahir']),
            'jabatan_pejabat' =>  $data['ref_pejabat']['jabatan'],
            'jenis_surat' =>  $data['ref_pejabat']['jenis_surat'],
            'nomor_surat' =>  $data['ref_pejabat']['nomor_surat'],
            'tanggal_surat' => $data['format_tanggal_indo']->format_tgl($data['ref_pejabat']['tanggal_surat']),
            'perihal_surat' =>  $data['ref_pejabat']['perihal_surat'],
            'jenis_surat_limit' => $data['ref_sk_limit']['jenis_surat_limit'],
            'nomor_surat_limit' => $data['ref_sk_limit']['nomor_surat_limit'],
            'tanggal_surat_limit' => $data['format_tanggal_indo']->format_tgl($data['ref_sk_limit']['tanggal_surat_limit']),
            'perihal_surat_limit' => $data['ref_sk_limit']['perihal_surat_limit'],
            'nama_alias_debitur' => $data['cetak_pk']['nama_alias_debitur'],
            'tempat_lahir_debitur' => $data['cetak_pk']['tempat_lahir_debitur'],
            'tanggal_lahir_debitur' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_lahir_debitur'])),
            'jenis_pekerjaan_debitur' => $data['cetak_pk']['jenis_pekerjaan_debitur'],
            'instansi_debitur' => $data['cetak_pk']['instansi_debitur'],
            'nomor_ktp_debitur' => $data['cetak_pk']['no_ktp_debitur'],
            'alamat_ktp_debitur' => $data['cetak_pk']['alamat_ktp_debitur'],
            'alamat_domisili_debitur' => $data['cetak_pk']['alamat_domisili_debitur'],
            'nama_alias_pasangan' => $data['cetak_pk']['nama_alias_pasangan'],
            'tempat_lahir_pasangan' => $data['cetak_pk']['tempat_lahir_pasangan'],
            'tanggal_lahir_pasangan' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_lahir_pasangan'])),
            'jenis_pekerjaan_pasangan' => $data['cetak_pk']['jenis_pekerjaan_pasangan'],
            'instansi_pasangan' => $data['cetak_pk']['instansi_pasangan'],
            'nomor_ktp_pasangan' => $data['cetak_pk']['no_ktp_pasangan'],
            'alamat_ktp_pasangan' => $data['cetak_pk']['alamat_ktp_pasangan'],
            'alamat_domisili_pasangan' => $data['cetak_pk']['alamat_domisili_pasangan'],
            'plafond' => number_format($data['cetak_pk']['plafond'], 0, ',', '.'),
            'plafond_terbilang' => preg_replace("/[[:blank:]]+/", " ", $data['terbilang']->terbilang(strval($data['cetak_pk']['plafond']))) . ' rupiah',
            'jangka_waktu_terbilang' => preg_replace("/[[:blank:]]+/", " ", $data['terbilang']->terbilang(strval($data['cetak_pk']['jangka_waktu']))),
            'suku_bunga_terbilang' => $data['terbilang']->terbilang($data['cetak_pk']['suku_bunga']) . ' persen',
            'persen_provisi_terbilang' => $data['terbilang']->terbilang($data['cetak_pk']['persen_provisi']) . ' persen',
            'persen_administrasi_terbilang' =>  $data['terbilang']->terbilang($data['cetak_pk']['persen_administrasi']) . ' persen',
            'angsuran_perbulan_terbilang' => $data['terbilang']->terbilang(strval($data['cetak_pk']['angsuran_perbulan'])) . ' rupiah',
            'persen_denda_terbilang' => $data['terbilang']->terbilang(strval($data['cetak_pk']['denda'])) . ' persen',
            'jenis_kredit' => $data['cetak_pk']['jenis_kredit'],
            'jangka_waktu' => $data['cetak_pk']['jangka_waktu'],
            'tanggal_mulai_jw' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_mulai_jw'])),
            'tanggal_akhir_jw' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_akhir_jw'])),
            'suku_bunga' => $data['cetak_pk']['suku_bunga'],
            'sistem_bunga' => $data['cetak_pk']['sistem_bunga'],
            'angsuran_perbulan' => number_format($data['cetak_pk']['angsuran_perbulan'], 0, ',', '.'),
            'persen_provisi' => $data['cetak_pk']['persen_provisi'],
            'persen_administrasi' => $data['cetak_pk']['persen_administrasi'],
            'tujuan_penggunaan_kredit' => $data['cetak_pk']['tujuan_penggunaan_kredit'],
            'kota_cabang' => $data['tbl_cabang']['kota'],
            'tanggal_mulai_angsuran' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_mulai_angsuran'])),
            'tanggal_angsuran' => $data['cetak_pk']['tgl_angsuran'],
            'tanggal_akhir_angsuran' => date('d-m-Y', strtotime($data['cetak_pk']['tgl_akhir_angsuran'])),
            'persen_denda' => $data['cetak_pk']['denda'],
            'tujuan_penggunaan_kredit' => $data['cetak_pk']['tujuan_penggunaan_kredit'],
            'pengadilan_negeri_cabang' => $data['tbl_cabang']['pengadilan_negeri'],
            'nomor_sppk' =>  $data['cetak_pk']['no_sppk'],
            'alamat_cabang' => $data['tbl_cabang']['alamat'],
            'telepon_cabang' => $data['tbl_cabang']['telephone'],
            'email_cabang' => $data['tbl_cabang']['email'],
            'telepon_debitur' => $data['tbl_permohon_kredit']['telepon_pemohon'],
            'media_sosial_debitur' => $data['tbl_permohon_kredit']['media_sosial'],
        ];

        $jaminan = [];
        if ($data['cetak_pk'] != null) {

            for ($a = 1; $a <= 20; $a++) {
                if ($data['cetak_pk']['jaminan_' . $a] != '') {
                    $jaminan[$a] = '- ' . $data['cetak_pk']['jaminan_' . $a];
                }
            }
        }

        $datatambahan = [
            'tgl_surat_kuasa_pasangan' => (isset($data['cetak_pk']['tgl_surat_kuasa_pasangan']) ? $data['format_tanggal_indo']->format_tgl($data['tgl_surat_kuasa_pasangan']) : ''),
            'tipe_pejabat' => $data['ref_pejabat']['tipe_pejabat'],
            'nama_pasangan' => $data['cetak_pk']['nama_pasangan'],
            'nama_pemohon' => $data['nama_pemohon'],
            'jenis_kelamin_debitur' => $data['cetak_pk']['jenis_kelamin_debitur'],
            'jaminan' => $jaminan
        ...
        return $data_cetak_pk;
    }


    public function format_data_cetak_pk($id)
    {
    ...
    public function rtf()
    {
        ...
        // Menggunakan file gambar.docx sebagai template
        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('cetak/cetakbaru/tes.docx');
        try {
            // Proses penggantian nilai pada template
            $templateProcessor->setValue('halo', 'HASMITRA');

            // Path gambar yang akan dimasukkan
            $imagePath = 'upload/tes.png';

            // Mengganti placeholder dengan gambar
            $templateProcessor->setImageValue('gambar', array('path' => $imagePath, 'width' => 300, 'height' => 200));

            // Simpan hasilnya ke dalam file output.docx
            $outputFilePath = 'cetak/cetakbaru/hasiltes.docx';
            $templateProcessor->saveAs($outputFilePath);

            // Setel header untuk pengunduhan
            header('Content-Type: application/octet-stream');
            header('Content-Disposition: attachment; filename="hasiltes.docx"');
            header('Content-Length: ' . filesize($outputFilePath));
    ...
    // public function spd_asuransi($id)
    // {
    ...
    //     $dataa['tbl_permohon_kredit'] = $this->model('m_cetak')->get_tbl_permohon_kredit($id);
    //     $nama_pemohon = $dataa['tbl_permohon_kredit']['nama_pemohon'];
    //     $nama_file1 = "SPD Asuransi";
    //     $ext = ".rtf";
    //     $nama_file2 = $nama_file1 . " " . $nama_pemohon . $ext;


    //     $document = file_get_contents("cetak/SPD Asuransi.rtf");
    ...
    public function analisa_keputusan($id)
    {
        ...
        // for ($a = 1; $a <= $batas; $a++) {
        //     $document = str_replace(("#jaminan_" . $a), $agunan[$a], $document);
        // }

        // for ($a = $batas + 1; $a <= 20; $a++) {

        //     if ($a < 10) {
        //         $document = str_replace(("#jaminan_" . $a), "", $document);
        //     } else {
        //         $document = str_replace(("#" . $a . "jaminan"), "", $document);
        //     }
        // }
    ...
    // save fungsi cetak yang tidak di pake lagi versi nya
