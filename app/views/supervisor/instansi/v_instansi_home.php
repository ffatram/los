...
<body class="hold-transition sidebar-mini layout-fixed">
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <div class="text-center">
                                                                        ...
                                                                        <button type="button" class="btn btn-danger btn-sm btn-hapus" data-id="<?= $i['id'] ?>" data-data1="<?= $i['kode_instansi'] ?>" data-data2="<?= $i['nama_instansi'] ?>" data-data3="<?= $i['kode_cabang'] ?>" title="Hapus"><i class="fas fa-trash-alt"></i></button>
                                                                    </div>
                        ...
                        <div class="col-12">
                            <form id="myformtambah">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="kode_instansi" class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" required>
                                </div>
                                <div class="form-group">
                        ...
                        <div class="col-12">
                            <form id="myformedit">
                                <div class="form-group">
                                    ...
                                    <label>Kode Instansi</label>
                                    <input type="text" name="kode_instansi" id="data1" class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');" required>
                                </div>
                                <div class="form-group">
                                    <label>Nama Instansi</label>
                                    <input type="text" name="nama_instansi" id="data2" class="form-control" required>
                                </div>

                                <div class="form-group">
                                    <label>Kode Cabang</label>
                                    <select name="kode_cabang" id="data3" class="form-control" required>
                                        <option value="" selected="true" disabled="disabled">- Silahkan Pilih -</option>
                                        <?php
                                        foreach ($data['daftar_cabang'] as $i) {
                                        ?>
    ...
    <script>
        $('#myformtambah').on('submit', function(e) {
            ...
            e.preventDefault()
            // tampung hasil ajax ke variabel 
            cek_inkaso_kode_cabang = ajax_cek("/supervisor/instansi_cek_data", myform);
            if (cek_inkaso_kode_cabang == "gagal") {
                Swal.fire({
                    ...
                    if (respon.isConfirmed) {
                        ...
                        var res = ajax_cek("/supervisor/instansi_tambah", myform);
            ...
            } else if (cek_inkaso_kode_cabang == "sukses") {
                Swal.fire(
                ...
                alert("Gagal : " + cek_inkaso_kode_cabang)
            }
        })
    </script>
    ...
    <script>
        $('.btn-edit').on('click', function() {
            ...
            $.ajax({
                url: '<?= BASEURL ?>/supervisor/get_data_instansi_edit_id',
                type: 'post',
                data: {
                    id: id
                },
                success: function(data) {
                    ...
                    $('#id').val(i.id);
                    $('#data1').val(i.kode_instansi);
                    $('#data2').val(i.nama_instansi);
                    $('#data3').val(i.kode_cabang);
                },
            });
    ...
    <script>
        ...
        $('#myformedit').on('submit', function(e) {
            ...
            if (data1 == $('#data1').val() && data3 == $('#data3').val()) {
                ...
                if (data2 == $('#data2').val()) {
                    ...
                    Swal.fire({
                        ...
                        if (respon.isConfirmed) {
                            ...
                            var hasil = ajax_cek("/supervisor/instansi_update", myform);
                            Swal.fire({
                                icon: 'success',
                ...
                var hasil = ajax_cek("/supervisor/instansi_cek_data", myform);
                if (hasil == "sukses") {
                    Swal.fire(
                        'Data sudah ada, ganti yang lain',
                        '',
                        'info'
                    )
                } else if (hasil == "gagal") {
    ...
    <script>
        $('.btn-hapus').on('click', function() {
            ...
            var data3 = $(this).attr('data-data3')
            ...
            Swal.fire({
                ...
                html: 'Kode Instansi : <b class="text-danger"> ' + data1 + '</b>' + '</br>' +
                    'Nama Instansi : <b class="text-danger"> ' + data2 + '</b>' + '</br>' +
                    'Kode Cabang : <b class="text-danger"> ' + data3 + '</b>'
            }).then((respon) => {
                if (respon.isConfirmed) {
                    $.ajax({
                        type: 'POST',
                        url: '<?= BASEURL . '/supervisor/instansi_hapus' ?>',
                        data: {
                            id: id
                        },
                        success: function(res) {
